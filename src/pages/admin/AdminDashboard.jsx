...
/* CSS VARIABLES — scoped via :where(.adm) */
:where(.adm){
...
.adm .adm-sb-glow{
  ...
  background:linear-gradient(90deg,var(--violet),var(--g1));
}
.adm .adm-profile{
...
.adm .adm-role-filter select{
  ...
  cursor:pointer;padding-right:2px;appearance:none;min-width:90px;
}
...
.adm .adm-top-date{
  ...
  background:var(--bg3);border:1px solid var(--border);
  border-radius:var(--r2);padding:5px 10px;white-space:nowrap;
...
.adm .btn-primary{
  background:linear-gradient(135deg,var(--violet),var(--g3));color:#fff;
...
.adm .approve-btn{
  ...
  color:#060c1a;border:none;padding:5px 11px;border-radius:7px;
...
.adm .struct-wrap{
  ...
  max-width:520px;position:relative;overflow:hidden;
}
...
.adm .struct-fld input:focus,.adm .struct-fld select:focus{
  border-color:rgba(0,212,160,0.4);box-shadow:0 0 0 3px rgba(0,212,160,0.07);
}
  ...
  return (
    <div className="adm-stat">
      <div className="adm-stat-top">
        ...
        {trend != null && (
          <span className="adm-stat-trend up">↑ {trend}%</span>
        )}
      </div>
        ...
        <div className="adm-modal-body">
          ...
          <div className="row2">
            <div className="fld"><label>N° dossier</label>
              <input value={form.numero_dossier || ""} placeholder=""
          ...
          <div className="row2">
            <div className="fld"><label>Département</label>
              <select value={form.departement_id || ""} disabled={!form.ufr_id}
            ...
            <div className="fld"><label>Filière</label>
              <select value={form.filiere_id || ""} disabled={!form.departement_id}
...
/* ─── STRUCT VIEW ─── */
function StructView({ type, ufrs, allDepartements,
  ufrForm, setUfrForm, deptForm, setDeptForm,
      ...
      fields: (
        <>
          ...
          <div className="struct-fld">
            ...
            <select onChange={e => setDeptForm({ ...deptForm, ufr_id: e.target.value })}>
      ...
      fields: (
        <>
          ...
          <div className="struct-fld">
            ...
            <select onChange={e => handleFiliereUfrChange(e.target.value)}>
              <option value="">Choisir une UFR</option>
              {ufrs.map(u => <option key={u.id} value={u.id}>{u.nom}</option>)}
            </select>
          </div>
          <div className="struct-fld">
            ...
            <select onChange={e => setFiliereForm({ ...filiereForm, departement_id: e.target.value })}>
              <option value="">Choisir un département</option>
              {filiereDepts.map(d => <option key={d.id} value={d.id}>{d.nom}</option>)}
            </select>
          </div>
        </>
      ),
...
export default function AdminDashboard() {
  ...
  useEffect(() => { load(); fetchUfrs(); fetchAllDepts(); }, []);
...
const load = async () => {
  ...
  const openEdit = (user) => { setSelectedUser(user); setEditOpen(true); };
  ...
  // ── Export Excel intelligent — vrais noms de rôle et statut ──
  const exportExcel = () => {
      ...
      <div className="adm-main">
            ...
            <div className="adm-srch">
              ...
              {search && (
                <span style={{ cursor: "pointer", color: "var(--ink4)", flexShrink: 0 }}
                  onClick={() => setSearch("")}><X size={11} /></span>
              )}
            </div>
        ...
        <div className="adm-content">
          ...
          {(view === "ufr" || view === "departement" || view === "filiere") && (
            <StructView
              type={view} ufrs={ufrs} allDepartements={allDepts}
      ...
      <AnimatePresence>
        {open && (
          <UserModal title="Créer un utilisateur"
            ...
            onSave={createUser} onClose={() => setOpen(false)} saveLabel="Créer" />
        )}
